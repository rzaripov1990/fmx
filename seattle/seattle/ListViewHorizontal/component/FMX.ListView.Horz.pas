...
type
  /// <summary>This is the base class for TListView controls. It provides the basic interface between TListView
  /// and TListViewAdapter. TListViewAdapter events are intercepted at this level and should be implemented
  /// by the descendants in their corresponding Do... virtual methods.</summary>
  TAdapterListViewHorz = class(TStyledControl)
  strict private
  ...
  protected
    ...
    procedure SetAdapter(const Adapter: IListViewAdapter);
  ...
  /// </summary>
  TListViewBaseHorz = class(TAdapterListViewHorz, ISearchResponder, IListItemStyleResources, IListViewController, IGlyph,
    IMessageSender)
  private const
  ...
  private
    ...
    FScrollingActive: Boolean;
    FScrollingMouseTouch: Boolean;
    FNeedShowScrollBar: Boolean;
    ...
    procedure StartIncident(const Incident: TDelayedIncident; const Triggered: Boolean = True; const TimeToWait: Single = 0;
    ...
    procedure DrawSelectedFill(const LocRect: TRectF; const Opacity: Single);
  ...
  protected
    ...
    function UsesNativePresentation: Boolean; virtual;
    { IGlyph }
  ...
  TPresentedListViewHorz = class(TListViewBaseHorz, IListViewPresentationParent)
  strict private
  ...
  protected
    ...
    procedure DoItemsChange; override;
    procedure DoItemsInvalidate; override;
    procedure DoListItemChange(const AItem: TListItem); override;
  ...
  TAppearanceListViewHorz = class(TPresentedListViewHorz, IAppearanceItemOwner, IPublishedAppearanceOwner)
  public type
  ...
  strict private
    ...
    procedure CheckableCheckChange(Sender: TObject);
    procedure DoResetViewAppearance(const AItem: TListViewItem);
  ...
  protected
    ...
    // Feedback from items with TListItemSimpleControl
  ...
  TCustomListViewHorz = class(TAppearanceListViewHorz)
  end;

  [ComponentPlatformsAttribute(fgAllPlatform)]
  TListViewHorz = class(TCustomListViewHorz)
  protected
...
type
  ...
  TEditModeOption = (DisallowSelection, HideSelection, RadioButtonMode, UncheckMode, ModalMode, ClearWhenStart, MultiSelect);
...
{$ENDREGION}
{$REGION 'TListViewBaseHorz'}

class function TListViewBaseHorz.TItemSelectionAlpha.Create(const StartTime: Double; const Alpha, StartAlpha: Single)
  : TItemSelectionAlpha;
begin
...
class function TListViewBaseHorz.GetDefaultMargins: TRectF;
begin
  Result := TRectF.Create(DefaultLeftMargin, 0, DefaultRightMargin, 0);
end;

constructor TListViewBaseHorz.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FWidthSums := TItemHeightSums.Create;
...
destructor TListViewBaseHorz.Destroy;
begin
  FMessageSender.Free;
...
function TListViewBaseHorz.IsRunningOnDesktop: Boolean;
begin
  Result := TOSVersion.Platform in [pfWindows, pfMacOS, pfLinux];
end;

function TListViewBaseHorz.HasTouchTracking: Boolean;
begin
  Result := (FSystemInformationService <> nil) and
    (TScrollingBehaviour.TouchTracking in FSystemInformationService.GetScrollingBehaviour);
end;

function TListViewBaseHorz.HasSearchFeatures: Boolean;
begin
  Result := ((FListingService <> nil) and (FListingService.GetSearchFeatures <> [])) or (csDesigning in ComponentState);
end;

function TListViewBaseHorz.HasSearchAsItem: Boolean;
begin
...
function TListViewBaseHorz.IsDeleteModeAllowed: Boolean;
begin
  Result := TListingEditModeFeature.Delete in FListingService.GetEditModeFeatures;
end;

function TListViewBaseHorz.IsEditMode: Boolean;
begin
  Result := FEditMode;
end;

function TListViewBaseHorz.HasStretchyScrolling: Boolean;
begin
...
function TListViewBaseHorz.HasButtonsInCells: Boolean;
begin
  Result := False;
end;

function TListViewBaseHorz.HasCheckboxMode: Boolean;
begin
  Result := False;
end;

function TListViewBaseHorz.HasDeletionEditMode: Boolean;
begin
  Result := False;
end;

function TListViewBaseHorz.HasPhysicsStretchyScrolling: Boolean;
begin
  Result := HasTouchTracking and (FPullToRefresh or HasStretchyScrolling);
end;

function TListViewBaseHorz.HasScrollingStretchGlow: Boolean;
begin
  Result := (FListingService <> nil) and (TListingTransitionFeature.ScrollGlow in FListingService.GetTransitionFeatures);
end;

function TListViewBaseHorz.HasPullRefreshStroke: Boolean;
begin
  Result := (FListingService <> nil) and not(TListingTransitionFeature.PullToRefresh in FListingService.GetTransitionFeatures) and
    ((FPullRefreshAnimation = TPullRefreshAnimation.Playing) or (GetPullRefreshStrength > 0));
end;

function TListViewBaseHorz.CanDisplaySelectionForItem(const Index: Integer; const Item: TListItem;
...
begin
  ...
  if LItem = nil then
    LItem := Adapter[Index];
  Result := ((FItemIndex = Index) and FShowSelection and (LItem.Count > 0) and (LItem.Purpose = TListItemPurpose.None) and
    (not FEditMode) and (FDeleteButtonIndex = -1)) or (HasCheckboxMode and IncludeMultiSelect and (LItem.Count > 0) and
...
function TListViewBaseHorz.GetDefaultSelectionAlpha: Single;
begin
  if FEditMode then
...
procedure TListViewBaseHorz.OnSearchEditResize(Sender: TObject);
begin
  InvalidateHeights;
  StartIncident(TDelayedIncident.Invalidate);
end;

procedure TListViewBaseHorz.OnSearchEditChange(Sender: TObject);
begin
  if Assigned(FOnSearchChange) then
    FOnSearchChange(Self);
end;

function TListViewBaseHorz.GetItemCount: Integer;
begin
  Result := Adapter.Count;
end;

procedure TListViewBaseHorz.DoItemsMayChange;
begin
  inherited;
  FItemSelectedBeforeChange := Selected;
end;

procedure TListViewBaseHorz.DoItemsCouldHaveChanged;
var
...
begin
  ...
  if (FItemSelectedBeforeChange <> nil) and (FItemSelectedBeforeChange.Index >= 0) and
    (FItemSelectedBeforeChange.Index < Adapter.Count) and (Adapter[FItemSelectedBeforeChange.Index] = FItemSelectedBeforeChange)
  then
...
procedure TListViewBaseHorz.DoItemsInvalidate;
begin
  inherited;
  Invalidate;
end;

procedure TListViewBaseHorz.ItemSpacesChange(Sender: TObject);
begin
  Invalidate;
end;

procedure TListViewBaseHorz.SetSideSpace(const Value: Integer);
var
  NewValue: Integer;
begin
...
procedure TListViewBaseHorz.SetTransparent(const Value: Boolean);
begin
...
function TListViewBaseHorz.ShouldHandleEvents: Boolean;
begin
  Result := True;
end;

procedure TListViewBaseHorz.SelectItem(const ItemIndex: Integer);
var
  HasChange: Boolean;
...
begin
  ...
  begin
    if Supports(Adapter, IListViewCheckProvider, Checkable) then
      Checkable[NewItemIndex] := FEditMode;
  end
  else
    NewItemIndex := -1;
...
procedure TListViewBaseHorz.UnselectItem(const ItemIndex: Integer);
var
  Checkable: IListViewCheckProvider;
begin
...
procedure TListViewBaseHorz.SetAlternatingColors(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.SetItemIndexInternal(const Value: Integer; const DisableSelection, DisableCrossfade: Boolean);
var
  NewValue: Integer;
begin
  NewValue := Value;
...
function TListViewBaseHorz.GetItemIndex: Integer;
begin
  Result := FItemIndex;
end;

procedure TListViewBaseHorz.SetItemIndex(const Value: Integer);
begin
...
procedure TListViewBaseHorz.SetEditMode(const Value: Boolean);
var
  LHandled: Boolean;
  Checkable: IListViewCheckProvider;
begin
...
procedure TListViewBaseHorz.WillEnterEditMode(const Animated: Boolean);
begin
end;

procedure TListViewBaseHorz.SetCanSwipeDelete(const Value: Boolean);
begin
  if FCanSwipeDelete <> Value then
    FCanSwipeDelete := Value;
end;

procedure TListViewBaseHorz.SetSearchVisible(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.SetSearchAlwaysOnTop(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.SetControlType(const Value: TControlType);
begin
...
procedure TListViewBaseHorz.SetNativeOptions(const Value: TListViewNativeOptions);
begin
...
procedure TListViewBaseHorz.SetOnFilter(const Value: TFilterEvent);
begin
  FOnFilter := Value;
  if FSearchEdit <> nil then
    FSearchEdit.OnFilter := OnFilter;
...
function TListViewBaseHorz.GetEditModeTransitionAlpha: Single;
begin
  Result := FEditModeTransitionAlpha;
end;

function TListViewBaseHorz.GetDeleteModeTransitionAlpha: Single;
begin
  Result := FDeleteModeTransitionAlpha;
end;

procedure TListViewBaseHorz.FeedbackItemClickChecked(const Item: TListItem; const Checked: Boolean);
begin
  if Item <> nil then
    if Checked then
      SetDeleteButtonIndex(Item.Index)
    else
      SetDeleteButtonIndex(-1);
end;

function TListViewBaseHorz.GetItemEditOffset(const Item: TListItem): Single;
var
...
function TListViewBaseHorz.GetItemDeleteCutoff(const Item: TListItem): Single;
begin
  if (Item <> nil) and ((Item.Index = FDeleteButtonIndex) or (Item.Index = FPrevDeleteButtonIndex)) and (FDeleteLayout <> nil)
  then
...
function TListViewBaseHorz.GetClientMargins: TRectF;
begin
  Result := LocalRect;
end;

function TListViewBaseHorz.GetItemCurrentSelectionAlpha(const Item: TListItem): Single;
begin
  if Item <> nil then
...
function TListViewBaseHorz.UsesNativePresentation: Boolean;
begin
  Result := False;
end;

procedure TListViewBaseHorz.DoEditModeChanging(var AHandled: Boolean);
begin
  if Assigned(FOnEditModeChanging) then
    FOnEditModeChanging(Self, AHandled);
end;

procedure TListViewBaseHorz.DoEditModeChange;
begin
  if Assigned(FOnEditModeChange) then
    FOnEditModeChange(Self);
end;

procedure TListViewBaseHorz.DoItemsChange;
begin
...
procedure TListViewBaseHorz.DoAdapterSet;
begin
  UpdateScrollingLimits;
end;

procedure TListViewBaseHorz.SetFilterPredicate(const Predicate: TPredicate<string>);
var
...
procedure TListViewBaseHorz.RebuildList;
begin
end;

procedure TListViewBaseHorz.StopPullRefresh;
begin
end;

procedure TListViewBaseHorz.SetItemSpaces(const Value: TBounds);
begin
  FItemSpaces.Assign(Value);
end;

function TListViewBaseHorz.GetDeleteButtonText: string;
begin
...
procedure TListViewBaseHorz.SetDeleteButtonText(const Value: string);
begin
...
function TListViewBaseHorz.DeleteButtonTextStored: Boolean;
begin
  Result := FDeleteButtonText <> DefaultDeleteButtonText;
end;

function TListViewBaseHorz.DeleteItem(const ItemIndex: Integer): Boolean;
begin
...
procedure TListViewBaseHorz.DoDeleteItem(const ItemIndex: Integer);
var
...
function TListViewBaseHorz.HasRecurrentTimerEvents: Boolean;
begin
  Result :=
  // Delayed Incidents
...
procedure TListViewBaseHorz.DestroyRecurrentTimer;
begin
...
procedure TListViewBaseHorz.UpdateRecurrentTimer;
var
  HasEvents: Boolean;
begin
...
procedure TListViewBaseHorz.RecurrentTimerEvent;
begin
...
procedure TListViewBaseHorz.StartIncident(const Incident: TDelayedIncident; const Triggered: Boolean; const TimeToWait: Single;
...
procedure TListViewBaseHorz.TriggerIncidents(const Incident: TDelayedIncident; const ResetStartupTime: Boolean);
var
  I: Integer;
  CurTime: Double;
  Entry: TDelayedIncidentEntry;
...
procedure TListViewBaseHorz.ProcessIncident(const Entry: TDelayedIncidentEntry);
begin
...
procedure TListViewBaseHorz.ProcessDelayedIncidents;
...
procedure TListViewBaseHorz.ProcessTransitionAnimation;
begin
  case FTransitionType of
    TTransitionType.EditMode:
      if FEditMode then
      begin
        FEditModeTransitionAlpha := Min(Abs(FTimerService.GetTick - FTransitionStartTime) / EditModeAnimationDuration, 1);
      ...
      begin
        FEditModeTransitionAlpha := Max(1 - (Abs(FTimerService.GetTick - FTransitionStartTime) / EditModeAnimationDuration), 0);
    ...
    TTransitionType.DeleteMode:
      begin
        if FDeleteButtonIndex <> -1 then
        begin
          FDeleteModeTransitionAlpha := Min(Abs(FTimerService.GetTick - FTransitionStartTime) / DeleteModeAnimationDuration, 1);
...
procedure TListViewBaseHorz.ProcessTapSelectItem;

  procedure SetNewItemIndex(AValue: Integer);
  var
    LAllowSelection: Boolean;
  begin
    LAllowSelection := True;
    FMouseSetItemIndex := AValue;
    ...
    begin
      SetItemIndex(AValue);
      TLinkObservers.ListSelectionChanged(Observers);
      DoListItemClick(Adapter[ItemIndex]);
    end;
  end;
...
procedure TListViewBaseHorz.ProcessSelectionAlphas;
type
...
procedure TListViewBaseHorz.InsertItemCrossFade(const Index: Integer; const ShowAnimation: Boolean);
var
...
procedure TListViewBaseHorz.RemoveItemCrossFade(const Index: Integer);
begin
  if FSelectionAlphas.ContainsKey(Index) then
    FSelectionAlphas.Remove(Index);
end;

function TListViewBaseHorz.GetItemSelectionAlpha(const Index: Integer): Single;
var
  ItemAlpha: TItemSelectionAlpha;
begin
...
procedure TListViewBaseHorz.InitEditModeAnimation;
var
  Checkable: IListViewCheckProvider;
begin
  if Supports(Adapter, IListViewCheckProvider, Checkable) then
...
procedure TListViewBaseHorz.ResetEditModeAnimation;
begin
  FTransitionType := TTransitionType.None;
  UpdateRecurrentTimer;
...
procedure TListViewBaseHorz.InitDeleteModeAnimation;
begin
...
procedure TListViewBaseHorz.ResetDeleteModeAnimation;
begin
  FTransitionType := TTransitionType.None;
  UpdateRecurrentTimer;
...
procedure TListViewBaseHorz.StartPullRefreshAnimation;
begin
  FPullRefreshAnimation := TPullRefreshAnimation.Playing;
...
procedure TListViewBaseHorz.ProcessPullRefreshAnimation;
var
  EndTrigger: Boolean;
begin
...
function TListViewBaseHorz.GetPullRefreshStrength: Single;
begin
...
function TListViewBaseHorz.GetPullRefreshIndicatorSteps: Integer;
const
  IndicatorStrengthPerStep = 5;
begin
  case FPullRefreshAnimation of
    { TListViewBaseHorz. } TPullRefreshAnimation.NotPlaying:
      Result := EnsureRange(Round((GetPullRefreshStrength - PullRefreshIndicatorStrengthStart) / IndicatorStrengthPerStep), 0,
        PullRefreshIndicatorMaxSteps);

    { TListViewBaseHorz. } TPullRefreshAnimation.Playing:
...
function TListViewBaseHorz.GetPullRefreshIndicatorAlpha: Single;
const
...
procedure TListViewBaseHorz.PaintPullRefreshIndicator(const ACanvas: TCanvas; const AStrength, AOpacity: Single);
const
...
function TListViewBaseHorz.GetPullRefreshStrokeWidth: Single;
const
...
begin
  if FPullRefreshAnimation = TPullRefreshAnimation.Playing then
    Result := Min(Power(Abs(FTimerService.GetTick - FPullRefreshAnimationStartTime) * StrokeCollapseSpeed1, StrokeCollapsePower) *
...
procedure TListViewBaseHorz.PaintPullRefreshStroke(const ACanvas: TCanvas; const AStrength, AOpacity: Single);
const
...
begin
  ...
  try
    ...
    if FPullRefreshAnimation = TPullRefreshAnimation.Playing then
    begin
      ACanvas.FillRect(TRectF.Create(0, TopAdjust, (Width - StrokeLength) / 2, TopAdjust + DefaultStrokeThickness), 0, 0,
        AllCorners, AOpacity, StrokeBrush);
      ACanvas.FillRect(TRectF.Create((Width + StrokeLength) / 2, TopAdjust, Width, TopAdjust + DefaultStrokeThickness), 0, 0,
        AllCorners, AOpacity, StrokeBrush);
    end;
...
procedure TListViewBaseHorz.PaintScrollingStretchGlow(const ACanvas: TCanvas; const AIntensity, AOpacity: Single);
var
...
procedure TListViewBaseHorz.UpdatePullRefreshState;
var
  Trigger: Boolean;
begin
  if FPullRefreshTriggered and (GetPullRefreshStrength < 1) and (FPullRefreshAnimation <> TPullRefreshAnimation.Playing) then
  begin
...
procedure TListViewBaseHorz.UpdateScrollStretchStrength(const NewValue: Single);
begin
...
procedure TListViewBaseHorz.ScrollStretchChanged;
begin
...
procedure TListViewBaseHorz.UpdateDeleteButtonLayout;
var
  RelRect: TRectF;
begin
  ...
  FDeleteLayout.Height := RelRect.Height;
  FDeleteLayout.Position.X := RelRect.Right;
  FDeleteLayout.Position.Y := (RelRect.Top + RelRect.Bottom - FDeleteLayout.Height) / 2;
end;

procedure TListViewBaseHorz.DoDeleteButtonClick(Sender: TObject);
begin
...
procedure TListViewBaseHorz.ProceedDeleteItem;
var
  Editor: IListViewEditor;
begin
...
procedure TListViewBaseHorz.SetDeleteButtonIndex(const NewItemIndex: Integer);
begin
...
function TListViewBaseHorz.CanObserve(const ID: Integer): Boolean;
begin
  Result := False;
...
procedure TListViewBaseHorz.ObserversBeforeSelection(out LAllowSelection: Boolean);
begin
  LAllowSelection := True;
...
procedure TListViewBaseHorz.DoListItemChange(const AListItem: TListItem);
begin
  if Assigned(FOnItemChange) then
    FOnItemChange(Self, AListItem);
end;

procedure TListViewBaseHorz.DoListItemClick(const AListItem: TListItem);
var
  Checkable: IListViewCheckProvider;
begin
  if (AListItem <> nil) and Supports(Adapter, IListViewCheckProvider, Checkable) then
    Checkable.SyncCheckedState(AListItem.Index);

  if Assigned(FOnListItemClick) then
    FOnListItemClick(Self, AListItem);
end;

procedure TListViewBaseHorz.DoUpdateItemView(const AListItem: TListItem);
begin
  if Assigned(FOnUpdateItemView) then
    FOnUpdateItemView(Self, AListItem);
end;

procedure TListViewBaseHorz.DoUpdatingItemView(const AListItem: TListItem; var AHandled: Boolean);
begin
  if Assigned(FOnUpdatingItemView) then
    FOnUpdatingItemView(Self, AListItem, AHandled);
end;

procedure TListViewBaseHorz.DoUpdateScrollViewPos(const Value: Single);
begin
end;

procedure TListViewBaseHorz.ReadCanSwipeDelete(Reader: TReader);
begin
  CanSwipeDelete := Reader.ReadBoolean;
end;

procedure TListViewBaseHorz.ReadIsSearchVisible(Reader: TReader);
begin
  SearchVisible := Reader.ReadBoolean;
end;

procedure TListViewBaseHorz.ReadIsSearchAlwaysOnTop(Reader: TReader);
begin
  SearchAlwaysOnTop := Reader.ReadBoolean;
end;

procedure TListViewBaseHorz.ReadEditModeOptions(Reader: TReader);
begin
  TOpenReader(Reader).ReadSet(TypeInfo(TEditModeOptions));
end;

procedure TListViewBaseHorz.DefineProperties(Filer: TFiler);
begin
  inherited;
...
procedure TListViewBaseHorz.DoRealign;
begin
  inherited;
  Repaint;
end;

procedure TListViewBaseHorz.DoExit;
begin
  inherited;
...
procedure TListViewBaseHorz.UpdateScrollViewPos(const Value: Single);
begin
...
procedure TListViewBaseHorz.UpdateSearchEditPos;
var
  NewPos: Single;
begin
  if FSearchEdit <> nil then
  begin
    if HasSearchAsItem then
      NewPos := Max(FSearchEdit.Width - FScrollViewPos, 0) - FSearchEdit.Width
    else
      NewPos := 0;

    if not SameValue(FSearchEdit.Position.X, NewPos, TEpsilon.Position) then
    begin
      FSearchEdit.Position.X := NewPos;
      Invalidate;
    end;
  end;
end;

function TListViewBaseHorz.GetMaxScrollViewPos: Integer;
begin
  Result := Max(Trunc(FSideSpace * 2 + FMaxKnownHeight - LocalRect.Width), 0);
end;

function TListViewBaseHorz.GetMessageSender: TMessageSender;
begin
...
procedure TListViewBaseHorz.SetScrollViewPos(const Value: Single);
var
...
procedure TListViewBaseHorz.SetSelected(const Value: TListItem);
begin
  if Value = nil then
    ItemIndex := -1
  else
...
procedure TListViewBaseHorz.SetSelectionCrossfade(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.SetPullToRefresh(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.SetShowSelection(const Value: Boolean);
begin
...
procedure TListViewBaseHorz.ScrollBarChange(Sender: TObject);
begin
...
procedure TListViewBaseHorz.AniCalcChange(Sender: TObject);
var
...
begin
  ...
  if (not SameValue(NewViewPos, FScrollViewPos, TEpsilon.Vector)) and FNeedShowScrollBar then
  begin
    FScrollBar.StopPropertyAnimation('Opacity');
    FScrollBar.Opacity := 1;

    FNeedShowScrollBar := False;
  end;

  if FScrollingActive then
  begin
...
procedure TListViewBaseHorz.AniCalcStart(Sender: TObject);
begin
  ...
  FNeedShowScrollBar := False; // Scroll
  FScrollingActive := True;
end;

procedure TListViewBaseHorz.AniCalcStop(Sender: TObject);
var
  ScrollPixelAlign: Boolean;
begin
  ScrollPixelAlign := FScrollingActive;

  FScrollingActive := False;
...
procedure TListViewBaseHorz.UpdateScrollBar;
var
...
begin
  ...
  try
    ...
    if not(csDesigning in ComponentState) then // Don't show at design time
      FScrollBar.Visible := (FMaxKnownHeight > LocalHeight) or (HasTouchTracking and FScrollingActive)
    else
      FScrollBar.Visible := False;
...
procedure TListViewBaseHorz.UpdateScrollingLimits;
var
...
procedure TListViewBaseHorz.GetNumberOfRenderingPasses(const StartItem, EndItem: Integer; var Passes, Subpasses: Integer);
var
  I, J: Integer;
  ListItem: TListItem;
begin
...
function TListViewBaseHorz.GetFinalItemSpaces(const ForceIncludeScrollBar: Boolean): TRectF;
begin
...
function TListViewBaseHorz.GetFinalItemSize(const ForceIncludeScrollBar: Boolean): TSizeF;
var
...
function TListViewBaseHorz.GetItemRelRect(const Index: Integer; const LocRect: TRectF; const SideSpace: Integer = 0): TRectF;
begin
  Result := RectF(LocRect.Left + FSideSpace + FWidthSums[Index] - FScrollViewPos, LocRect.Top + FSideSpace + SideSpace,
    GetItemHeight(Index) - FSideSpace, LocRect.Height - ((SideSpace + FSideSpace) * 2));

  if (FScrollBar <> nil) and (not HasTouchTracking) and FScrollBar.Visible then
    Result.Bottom := Result.Bottom - FScrollBar.Height;
end;

function TListViewBaseHorz.GetSelected: TListItem;
begin
...
function TListViewBaseHorz.GetStyleResources: TListItemStyleResources;
begin
  ApplyStyleLookup;
  Result := FStyleResources;
end;

function TListViewBaseHorz.StyleResourcesNeedUpdate: Boolean;
begin
  Result := FUpdatingStyleResources;
end;

function TListViewBaseHorz.GetItemGroupSeparators(const Index: Integer): Integer;
var
  EndIndex: Integer;
  Prev, Next: TListItem;
begin
...
function TListViewBaseHorz.GetItemHeight(const Index: Integer): Integer;
begin
...
function TListViewBaseHorz.GetSeparatorLineHeight: Single;
begin
...
function TListViewBaseHorz.AlignValueToPixel(const Value: Single): Single;
begin
  if FScrollScale > TEpsilon.Scale then
...
procedure TListViewBaseHorz.DrawItemsFill(const StartItem, EndItem: Integer; const LocRect: TRectF; const Opacity: Single;
...
begin
  ...
  for I := StartItem to EndItem do
    ...
    begin
      ...
      if (ListItem <> nil) and ((ListItem.Count > 0) or (Adapter[I - 1].Purpose <> TListItemPurpose.None)) then
      begin
        ...
        begin
          ...
          // отрисовываем кастомный цвет....................................ZuBy...
          ...
          { if TListViewItem(ListItem).Data['aUseBorderColor'].AsBoolean then
            begin
            FBrush.Color := TListViewItem(ListItem).Data['aBorderColor'].AsInteger;
            DrawRect.Top := DrawRect.Top + 5;
            DrawRect.Bottom := DrawRect.Bottom - 5;
            DrawRect.Left := DrawRect.Left + 5;
            DrawRect.Right := DrawRect.Right - 5;

            Canvas.Fill.Color := FBrush.Color;
            Canvas.DrawRect(DrawRect, 0, 0, AllCorners, Opacity);
            end; }
          // ...............................................................ZuBy...
        end;
        ...
        begin
          ...
          SepRect.Right := SepRect.Left + SepHeight;
          SepRect.Top := DrawRect.Top;
          SepRect.Bottom := DrawRect.Bottom;
        ...
        begin
          SepRect.Left := AlignValueToPixel(DrawRect.Left - SepHeight);
          SepRect.Right := SepRect.Left + SepHeight;
          SepRect.Top := DrawRect.Top - 1;
          SepRect.Bottom := DrawRect.Bottom + 1;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if I = 0 then
              DrawSubRect.Left := DrawSubRect.Left + 1;
...
procedure TListViewBaseHorz.DrawIndexFill(const AIndex: Integer; const LocRect: TRectF; const Opacity: Single);
var
  DrawRect: TRectF;
  SepHeight: Single;
begin
  ...
  if (AIndex >= Adapter.Count - 1) or (Adapter[AIndex + 1].Purpose = TListItemPurpose.None) then
    DrawRect.Top := DrawRect.Top - SepHeight;

{$IFDEF MSWINDOWS}
  // The selection seems to be broken on Windows (looks ugly, needs fixing). Meanwhile, attempt a temporal fix.
  if FItemBoxLight = nil then // ZuBy
    DrawRect.Inflate(-2, -2);
{$ENDIF}
  if FSelectionStyleImage <> nil then
  begin
    DrawRect.Left := AlignValueToPixel(DrawRect.Left - SepHeight) + SepHeight;
    DrawRect.Right := AlignValueToPixel(DrawRect.Right);
...
procedure TListViewBaseHorz.DrawSelectedFill(const LocRect: TRectF; const Opacity: Single);
begin
  DrawIndexFill(FItemIndex, LocRect, Opacity);
end;

function TListViewBaseHorz.GetHeaderRelRect(const StartItem, HeaderIndex: Integer; const LocRect: TRectF;
const SideSpace: Integer): TRectF;
var
  LimitRect: TRectF;
begin
  Result := GetItemRelRect(HeaderIndex, LocRect, SideSpace);
  if Result.Left < LocRect.Left then
  ...
  begin
    ...
    if Result.Right > LimitRect.Right then
      Result.Offset(0, LimitRect.Right - Result.Right);
  end;
end;

procedure TListViewBaseHorz.DrawHeaderItem(const LocRect: TRectF; const StartItem, HeaderIndex: Integer; const Opacity: Single);
begin
  if FHeaderStyleImage <> nil then
    FHeaderStyleImage.DrawToCanvas(Canvas, GetHeaderRelRect(StartItem, HeaderIndex, LocRect), Opacity);
end;

function TListViewBaseHorz.GetItemClientRect(const Index: Integer): TRectF;
var
  MarginSize: TPointF;
  FinalItemSpaces: TRectF;
begin
  ...
  Result.Right := GetItemHeight(Index) - MarginSize.X - (FSideSpace * 2);
  Result.Bottom := Height - (FSideSpace + FinalItemSpaces.Bottom);
...
procedure TListViewBaseHorz.DrawListItems(const AbsOpacity: Single);
...
var
  I, StartItem, EndItem, MaxWidth, ItemHeaderIndex, SubPassNo: Integer;
...
begin
  ...
  LocRect := LocalRect;
  // LocRect := RectF(0, 0, Width, Height);
  ClipRect := LocRect;
  ...
  if FSearchVisible and (FSearchEdit <> nil) and (FListingService <> nil) then
    if (FSearchAlwaysOnTop or (not(TListingSearchFeature.AsFirstItem in FListingService.GetSearchFeatures))) and
      ...
      ClipRect.Top := ClipRect.Top { + Max(FSearchEdit.Height - FScrollViewPos, 0) }; // ????
  ...
  MaxWidth := Ceil(LocRect.Width + FScrollViewPos);
  ...
  for I := StartItem + 1 to MaxItemIndex - 1 do
    if FWidthSums[I + 1] > MaxWidth then
    begin
      EndItem := I;
      Break;
    end;
  ...
  if (TAlphaColorRec(FItemStyleFillColor).A > 0) or (FAlternatingColors and (TAlphaColorRec(FItemStyleFillAltColor).A > 0)) then
  begin
    if (TAlphaColorRec(FItemStyleFillColor).A >= 255) and
      (not FAlternatingColors or (TAlphaColorRec(FItemStyleFillAltColor).A >= 255)) and SameValue(AbsOpacity, 1, TEpsilon.Vector)
    then
    begin
  ...
  if (not FEditMode) and (FItemIndex >= StartItem) and (FItemIndex <= EndItem) and (FSelectionAlphas.Count < 1) and
    ...
    DrawSelectedFill(LocRect, AbsOpacity);

  if (not HasTouchTracking) and FEditMode and (FItemIndex >= StartItem) and (FItemIndex <= EndItem) then
    DrawIndexFill(FItemIndex, LocRect, AbsOpacity * 0.5);

  if HasCheckboxMode or (FSelectionAlphas.Count > 0) then
    for I := StartItem to EndItem do
      if CanDisplaySelectionForItem(I, nil, True, True) then
          ...
          begin
            ...
            begin
              if I < MaxItemIndex then
                ItemHeight := FWidthSums[I + 1] - FWidthSums[I]
              else
                ItemHeight := GetItemHeight(I);

              BorderRect := GetItemRelRect(I, LocRect); // ZuBy fix

              aO := TPointF.Create(BorderRect.Left + FSideSpace, BorderRect.Top + FSideSpace);
              CurDrawable.CalculateLocalRect(TRectF.Create(aO, BorderRect.Width - (FSideSpace * 2),
                BorderRect.Height - (FSideSpace * 2)), SceneScale, DrawStates);
            end;

            CurDrawable.Render(Canvas, I, DrawStates, SubPassNo);
          end;
        end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            CurDrawable.CalculateLocalRect(RelRect, SceneScale, []);
          end;

          CurDrawable.Render(Canvas, ItemHeaderIndex, [], SubPassNo);
        end;
      end;
  end;
...
procedure TListViewBaseHorz.Paint;
var
  LOpacity: Single;
begin
...
procedure TListViewBaseHorz.RecreateNativePresentation;
begin
end;

procedure TListViewBaseHorz.Loaded;
begin
  inherited;
  ImagesChanged;
  RecreateNativePresentation;
...
procedure TListViewBaseHorz.Resize;
begin
  inherited;
  BeginUpdate;
  try
...
procedure TListViewBaseHorz.EndUpdate;
begin
  inherited;
  if not IsUpdating and FNeedRebuild then
  begin
    RebuildList;
    FNeedRebuild := False;
  end;
...
function TListViewBaseHorz.ObjectAtPoint(P: TPointF): IControl;
var
  LocalPt: TPointF;
  ItemAt: Integer;
  Control: TControl;
...
function TListViewBaseHorz.FindLocalItemObjectAtPosition(const ItemIndex: Integer; const Position: TPointF): TListItemDrawable;
var
  I: Integer;
  Item: TListItem;
begin
...
procedure TListViewBaseHorz.KeyDown(var Key: Word; var KeyChar: System.WideChar; Shift: TShiftState);
var
  ...
  begin
    ...
    for I := LFirstVisible + 1 to Adapter.Count - 2 do
      if FWidthSums[I + 1] >= MaxHeight then
      begin
        LLastVisible := I;
        Break;
      end;
...
procedure TListViewBaseHorz.MouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
  ...
  begin
    ...
    begin
      ...
      else if Supports(Adapter, IListViewCheckProvider, Checkable) then
      begin
        FirstIndex := Checkable.FirstChecked(True);
        GlyphButtonDrawable := GetGlyphButton(FirstIndex);
        if (FirstIndex <> -1) and (not FScrollingActive) and (GlyphButtonDrawable <> nil) and
...
begin
  ...
  if (FAniCalc <> nil) and FScrollingActive and FAniCalc.Animation then
  begin
  ...
  begin
    ...
    begin
      ...
      if (not FScrollingActive) and FAutoTapScroll and (Distance < FAutoTapTreshold) then
        FAutoTapDistance := Round(FScrollViewPos)
      else
        FAutoTapDistance := 0;
    end;

    if not FScrollingActive then
    begin
      ...
      begin
        ...
        begin
          if Adapter[ItemAt].HasClickOnSelectItems then
            FItemIndex := -1;

          SetNewItemIndex(ItemAt);
          FMouseClickIndex := -1;
        end;
      end;
    end
    else
      FScrollingMouseTouch := True;
...
procedure TListViewBaseHorz.MouseMove(Shift: TShiftState; X, Y: Single);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      else if Abs(FMouseClickDelta.X) > MinScrollThreshold then
      begin
  ...
  if (not FEditMode) and (FDragListMode = TInternalDragMode.Swype) and (not FScrollingMouseTouch) and (FDeleteButtonIndex = -1)
    and HasTouchTracking then
  begin
...
procedure TListViewBaseHorz.MouseUp(Button: TMouseButton; Shift: TShiftState; X, Y: Single);
...
begin
  ...
  if (FAniCalc <> nil) and FScrollingActive then
    FAniCalc.MouseUp(X, Y);
  ...
  begin
    ...
    if not FScrollingMouseTouch then
    begin
      ...
      begin
        ...
        begin
          TAnimator.AnimateFloat(Self, 'ScrollViewPos', 0, Min(FAutoTapDistance / AutoTapScrollingSpeed, AutoTapMaxScrollingTime),
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              begin
                if HasTouchTracking and (FMouseClickIndex = -1) and Adapter[NewIndex].HasClickOnSelectItems then
                  FItemIndex := -1;
...
procedure TListViewBaseHorz.MouseWheel(Shift: TShiftState; WheelDelta: Integer; var Handled: Boolean);
var
  Offset: Single;
begin
  inherited;
...
procedure TListViewBaseHorz.DoMouseLeave;
begin
  inherited;
...
procedure TListViewBaseHorz.DoResetEditModeAnimation;
begin
end;

procedure TListViewBaseHorz.DoSetItemIndexInternal(const Value: Integer);
begin
end;

procedure TListViewBaseHorz.DoSetScrollViewPos(const Value: Single);
begin
end;

procedure TListViewBaseHorz.Invalidate;
begin
  InvalidateRect(LocalRect);
end;

procedure TListViewBaseHorz.DoChange;
begin
...
procedure TListViewBaseHorz.DoChangeRepainted;
begin
  if Assigned(FOnChangeRepainted) then
    FOnChangeRepainted(Self);
end;

procedure TListViewBaseHorz.UpdateItemLookups;
var
...
begin
  ...
  begin
    ...
    begin
      ...
      if Adapter[I].Purpose = TListItemPurpose.Header then
      begin
        CurHeaderRef := I;
        Adapter[I].HeaderRef := I;
      end;

      if Adapter[I].Purpose = TListItemPurpose.None then
      begin
        if Adapter[I].Count > 0 then
...
function TListViewBaseHorz.FindItemAbsoluteAt(const ViewAt: Integer): Integer;
var
...
begin
  ...
  if ViewAt < 1 then
    Exit(0);
  if ViewAt >= FWidthSums[FWidthSums.Count - 1] then
    Exit(FWidthSums.Count - 1);

  Left := 0;
  Right := FWidthSums.Count - 1;
  Pivot := 0;
  ...
  if (Result > 0) and (Result < FWidthSums.Count) and (FWidthSums[Result] >= ViewAt) then
    Dec(Result);
end;

function TListViewBaseHorz.FindItemAbsoluteAtWithCheck(const ViewAt: Integer): Integer;
begin
  Result := -1;
  if Adapter.Count > 0 then
  begin
    UpdateItemLookups;
    if ViewAt < FWidthSums[FWidthSums.Count - 1] + GetItemHeight(Adapter.Count - 1) then
      Result := FindItemAbsoluteAt(ViewAt);
  end;
end;

function TListViewBaseHorz.GetDefaultStyleLookupName: string;
begin
  Result := 'listviewstyle';
end;

procedure TListViewBaseHorz.ApplyStyle;
...
procedure TListViewBaseHorz.FreeStyle;
begin
  inherited;
end;

function TListViewBaseHorz.GetItemRect(const AItemIndex: Integer): TRectF;
begin
...
procedure TListViewBaseHorz.ScrollTo(const AItemIndex: Integer);
var
...
begin
  ...
  begin
    NewPos := FSideSpace + FWidthSums[AItemIndex];
    ...
    SetScrollViewPos(FSideSpace + FWidthSums[AItemIndex] - (LocRect.Width - GetItemHeight(AItemIndex)));
end;

procedure TListViewBaseHorz.ImagesChanged;
begin
  if ([csLoading, csDestroying] * ComponentState = []) and (FImageLink <> nil) then
    Invalidate;
end;

function TListViewBaseHorz.GetImages: TCustomImageList;
begin
...
procedure TListViewBaseHorz.SetImages(const Value: TCustomImageList);
begin
  if FImageLink <> nil then
    FImageLink.Images := Value;
end;

function TListViewBaseHorz.GetImageIndex: TImageIndex;
begin
  Result := -1;
end;

procedure TListViewBaseHorz.SetImageIndex(const Value: TImageIndex);
begin
  // none
end;

function TListViewBaseHorz.GetImageList: TBaseImageList;
begin
  Result := GetImages;
end;

procedure TListViewBaseHorz.SetImageList(const Value: TBaseImageList);
begin
...
destructor TPresentedListViewHorz.Destroy;
begin
  FPresentation := nil;
  inherited;
end;

procedure TPresentedListViewHorz.PMAncesstorPresentationLoaded(var AMessage: TDispatchMessageWithValue<Boolean>);
begin
  ExecuteInterlocked(
    procedure
    begin
...
procedure TPresentedListViewHorz.RecreateNativePresentation;
var
  PresentationService: IFMXListViewPresentationService;
  LPresentation: IInterface;
begin
  if (FControlType = TControlType.Platform) and (not(csDesigning in ComponentState)) and
  ...
  begin
    ...
    Supports(LPresentation, IListViewPresentation, FPresentation);
    if (FItemIndex <> -1) and not FEditMode then
...
procedure TPresentedListViewHorz.ChangeOrder;
begin
  inherited;
  ExecuteInterlocked(
    procedure
...
procedure TPresentedListViewHorz.AncestorVisibleChanged;
begin
  inherited;
  ExecuteInterlocked(
    procedure
...
procedure TPresentedListViewHorz.RecalcOpacity;
begin
  inherited;
  ExecuteInterlocked(
    procedure
...
procedure TPresentedListViewHorz.Paint;
begin
...
procedure TPresentedListViewHorz.PaintChildren;
const
  LabelMargins = 3;
  LabelPadding = 3;
  ...
  begin
    ...
    TextRect := TRectF.Create(TPointF.Create(Width - TextWidth - LabelMargins, Height - TextHeight - LabelMargins), TextWidth,
      TextHeight);
...
procedure TPresentedListViewHorz.ParentChanged;
begin
  inherited;
  ExecuteInterlocked(
    procedure
...
procedure TPresentedListViewHorz.RebuildList;
begin
  inherited;
...
procedure TPresentedListViewHorz.StopPullRefresh;
begin
  if FPresentation <> nil then
    FPresentation.StopPullRefresh;
end;

function TPresentedListViewHorz.GetRootObject: TObject;
begin
  if Root <> nil then
...
function TPresentedListViewHorz.GetAdapter: IListViewAdapter;
begin
  Result := Adapter;
end;

function TPresentedListViewHorz.GetContentFrame: TRect;
begin
  Result := GetBoundsRect.Round;
end;

function TPresentedListViewHorz.GetControlOpacity: Single;
begin
  Result := AbsoluteOpacity;
end;

function TPresentedListViewHorz.GetItemText(const ItemIndex: Integer): string;
var
...
function TPresentedListViewHorz.GetItemIndexTitle(const ItemIndex: Integer): string;
var
  Provider: IListViewTextProvider;
begin
  Result := string.Empty;
...
function TPresentedListViewHorz.CanSelectItem(const AItemIndex: Integer): Boolean;
begin
  Result := True;
...
procedure TPresentedListViewHorz.DidSelectItem(const AItemIndex: Integer);
begin
  ExecuteInterlocked(
    procedure
    begin
...
function TPresentedListViewHorz.CanUnselectItem(const AItemIndex: Integer): Boolean;
begin
  ObserversBeforeSelection(Result);
end;

procedure TPresentedListViewHorz.DidUnselectItem(const AItemIndex: Integer);
begin
  ExecuteInterlocked(
    procedure
    begin
...
function TPresentedListViewHorz.UsesNativePresentation: Boolean;
begin
  Result := FPresentation <> nil;
end;

procedure TPresentedListViewHorz.DoDeleteItem(const ItemIndex: Integer);
begin
  inherited;
  if FPresentation <> nil then
    FPresentation.ItemsUpdated;
...
procedure TPresentedListViewHorz.DoEditModeChange;
begin
  inherited;
  if FPresentation <> nil then
...
procedure TPresentedListViewHorz.DoResetEditModeAnimation;
begin
  inherited;
  if FPresentation <> nil then
    FPresentation.EditModeChanged;
...
procedure TPresentedListViewHorz.DoItemsChange;
begin
  InvalidateHeights;
  inherited;
  if FPresentation = nil then
...
procedure TPresentedListViewHorz.DoItemsInvalidate;
begin
  inherited;
  if IsUpdating then
    FNeedRebuild := True
...
procedure TPresentedListViewHorz.DoListItemChange(const AItem: TListItem);
var
  Checkable: IListViewCheckProvider;
begin
  if Supports(Adapter, IListViewCheckProvider, Checkable) then
...
procedure TPresentedListViewHorz.DoSetItemIndexInternal(const Value: Integer);
begin
  inherited;
  ExecuteInterlocked(
    procedure
...
procedure TPresentedListViewHorz.DoSetScrollViewPos(const Value: Single);
begin
  inherited;
  if FPresentation = nil then
  begin
...
procedure TPresentedListViewHorz.DoUpdateScrollViewPos(const Value: Single);
begin
  if FPresentation = nil then
    Invalidate;
end;

procedure TPresentedListViewHorz.ItemButtonClicked(const ItemIndex: Integer);
var
...
procedure TPresentedListViewHorz.InvokePullRefresh;
begin
  TThread.Queue(nil,
    procedure
    begin
...
procedure TPresentedListViewHorz.SetSearchFilter(const Filter: string);
var
  LText: string;
begin
...
function TPresentedListViewHorz.GetTableViewFlags: TListViewModeFlags;
begin
  Result := [];
  if FEditMode then
...
function TPresentedListViewHorz.GetTableViewOptions: TListViewNativeOptions;
begin
  Result := FNativeOptions;
end;

procedure TPresentedListViewHorz.RecalcEnabled;
begin
  inherited;
  if FPresentation <> nil then
...
function TPresentedListViewHorz.ShouldHandleEvents: Boolean;
begin
  Result := FPresentation = nil;
end;

procedure TPresentedListViewHorz.Show;
begin
  inherited;
  if FPresentation <> nil then
    FPresentation.StatusChanged;
...
procedure TPresentedListViewHorz.Hide;
begin
  inherited;
  if FPresentation <> nil then
    FPresentation.StatusChanged;
...
procedure TPresentedListViewHorz.Resize;
begin
  inherited;
...
procedure TPresentedListViewHorz.DoItemsResize;
begin
  inherited;
  if IsUpdating then
    FNeedRebuild := True
...
procedure TPresentedListViewHorz.ExecuteInterlocked(const P: TProc);
begin
...
constructor TAppearanceListViewHorz.Create(AOwner: TComponent);
begin
  inherited;
  ...
  FAppearanceViewItems := TAppearanceListViewItems.Create(Self);
  SetAdapter(FAppearanceViewItems);

  FAppearanceViewItems.OnNotify := ObjectsNotify;

  (FAppearanceViewItems as IListViewEditor).OnBeforeItemAdded := EditorBeforeItemAdded;
  (FAppearanceViewItems as IListViewEditor).OnAfterItemAdded := EditorAfterItemAdded;
  (FAppearanceViewItems as IListViewEditor).OnBeforeItemDeleted := EditorBeforeItemDeleted;
  (FAppearanceViewItems as IListViewEditor).OnAfterItemDeleted := EditorAfterItemDeleted;
  (FAppearanceViewItems as IListViewCheckProvider).OnCheckChange := CheckableCheckChange;
end;

destructor TAppearanceListViewHorz.Destroy;
begin
...
function TAppearanceListViewHorz.GetAppearanceListViewItem(const Index: Integer): TListViewItem;
begin
  Result := FAppearanceViewItems.AppearanceItem[Index];
end;

function TAppearanceListViewHorz.GetHeaderAppearanceName: string;
begin
  Result := FHeaderAppearanceProperties.Name;
end;

function TAppearanceListViewHorz.GetHeaderAppearanceProperties: TItemAppearanceProperties;
begin
  Result := FHeaderAppearanceProperties;
end;

function TAppearanceListViewHorz.GetHeaderAppearanceClassName: string;
begin
  Result := FHeaderAppearanceProperties.AppearanceClassName;
end;

function TAppearanceListViewHorz.GetFooterAppearanceName: string;
begin
  Result := FFooterAppearanceProperties.Name;
end;

function TAppearanceListViewHorz.GetFooterAppearanceProperties: TItemAppearanceProperties;
begin
  Result := FFooterAppearanceProperties;
end;

function TAppearanceListViewHorz.GetFooterAppearanceClassName: string;
begin
  Result := FFooterAppearanceProperties.AppearanceClassName;
end;

function TAppearanceListViewHorz.GetItemAppearanceName: string;
begin
  Result := FItemAppearanceProperties.Name;
end;

function TAppearanceListViewHorz.GetItemEditAppearanceName: string;
begin
  Result := FItemEditAppearanceProperties.Name;
end;

function TAppearanceListViewHorz.GetItemAppearanceProperties: TItemAppearanceProperties;
begin
  Result := FItemAppearanceProperties;
end;

function TAppearanceListViewHorz.GetItemEditAppearanceProperties: TItemAppearanceProperties;
begin
  Result := FItemEditAppearanceProperties;
end;

function TAppearanceListViewHorz.GetItemObjectsClassName: string;
begin
  Result := FItemAppearanceProperties.AppearanceClassName;
end;

function TAppearanceListViewHorz.HasButtonsInCells: Boolean;
begin
  Result := ItemAppearance.ItemAppearance = TAppearanceNames.ImageListItemRightButton;
end;

function TAppearanceListViewHorz.HasCheckboxMode: Boolean;
begin
  Result := FEditMode and (FAppearanceAllowsCheckboxes or ((not IsDeleteModeAllowed) and HasDeletionEditMode));
end;

function TAppearanceListViewHorz.HasDeletionEditMode: Boolean;
begin
  Result := FAppearanceAllowsDeleteMode;
end;

procedure TAppearanceListViewHorz.SetItemHeight(const Value: Integer);
begin
  FItemAppearanceProperties.Height := Value;
end;

procedure TAppearanceListViewHorz.SetItemEditHeight(const Value: Integer);
begin
  FItemEditAppearanceProperties.Height := Value;
end;

procedure TAppearanceListViewHorz.SetHeaderHeight(const Value: Integer);
begin
  FHeaderAppearanceProperties.Height := Value;
end;

procedure TAppearanceListViewHorz.SetFooterHeight(const Value: Integer);
begin
  FFooterAppearanceProperties.Height := Value;
end;

function TAppearanceListViewHorz.GetItemEditObjectsClassName: string;
begin
  Result := FItemEditAppearanceProperties.AppearanceClassName;
end;

procedure TAppearanceListViewHorz.SetFooterAppearanceClassName(const Value: string);
begin
  FFooterAppearanceProperties.AppearanceClassName := Value;
end;

procedure TAppearanceListViewHorz.SetAppearanceProperties(const Value: TPublishedAppearance);
begin
  Assert(False);
  // Do nothing
end;

procedure TAppearanceListViewHorz.SetItemAppearanceObjects(const Value: TPublishedObjects);
begin
  Assert(False);
  // Do nothing
end;

procedure TAppearanceListViewHorz.SetItemObjectsClassName(const Value: string);
begin
  FItemAppearanceProperties.AppearanceClassName := Value;
end;

procedure TAppearanceListViewHorz.SetItemEditObjectsClassName(const Value: string);
begin
  FItemEditAppearanceProperties.AppearanceClassName := Value;
end;

procedure TAppearanceListViewHorz.SetHeaderAppearanceName(const Value: string);
begin
...
procedure TAppearanceListViewHorz.SetHeaderAppearanceClassName(const Value: string);
begin
  FHeaderAppearanceProperties.AppearanceClassName := Value;
end;

procedure TAppearanceListViewHorz.SetFooterAppearanceName(const Value: string);
begin
...
function TAppearanceListViewHorz.GetAppearanceProperties: TArray<TItemAppearanceProperties>;
begin
  SetLength(Result, 4);
...
procedure TAppearanceListViewHorz.SetItemAppearanceName(const Value: string);
begin
...
procedure TAppearanceListViewHorz.SetItemEditAppearanceName(const Value: string);

  function AllowsCheckboxes(const Name: string): Boolean;
  begin
    Result := (Name = TAppearanceNames.ListItemRightDetailShowCheck) or (Name = TAppearanceNames.ImageListItemShowCheck) or
      (Name = TAppearanceNames.ImageListItemRightButtonShowCheck) or (Name = TAppearanceNames.ListItemShowCheck) or
...
procedure TAppearanceListViewHorz.InitializeItemAppearance(const AAppearance: TItemAppearanceProperties);
begin
  AAppearance.AppearanceClass := TNullItemObjects;
end;

procedure TAppearanceListViewHorz.ItemAppearanceChange(const Sender: TItemAppearanceProperties);
begin
...
procedure TAppearanceListViewHorz.ItemAppearanceChangeObjects(const Sender: TItemAppearanceProperties);
begin
  ItemAppearanceChange(Sender);
end;

procedure TAppearanceListViewHorz.ObjectsNotify(Sender: TObject; const Item: TListItem; Action: TCollectionNotification);
begin
...
procedure TAppearanceListViewHorz.AppearanceResetObjects(APurposes: TListItemPurposes);
var
...
procedure TAppearanceListViewHorz.ApplyStyle;
begin
  inherited;
  UpdateAppearanceStyleResources;
end;

procedure TAppearanceListViewHorz.AppearanceResetHeights(APurposes: TListItemPurposes);
var
  LPurposes: TListItemPurposes;
  LProperties: TItemAppearanceProperties;
begin
...
procedure TAppearanceListViewHorz.DoResetView(const Item: TListItem);
begin
  inherited;
  DoResetViewAppearance(TListViewItem(Item));
end;

procedure TAppearanceListViewHorz.DoResetViewAppearance(const AItem: TListViewItem);
var
  LHandled: Boolean;
...
begin
  ...
  try
    ...
    begin
      ...
      LItemObjects.OnButtonClick := Self.DoButtonClick;
      LItemObjects.OnButtonChange := Self.DoButtonChange;
...
procedure TAppearanceListViewHorz.RefreshAppearances(const APurposes: TListItemPurposes);
begin
  Adapter.ResetViews(APurposes);
  InvalidateHeights; // Object heights may have changed
end;

procedure TAppearanceListViewHorz.UpdateAppearanceStyleResources;
begin
...
procedure TAppearanceListViewHorz.WillEnterEditMode(const Animated: Boolean);
begin
  if Animated then
    EditModeAppearances
  else
...
procedure TAppearanceListViewHorz.ItemAppearanceChangeHeight(const Sender: TItemAppearanceProperties);
begin
  Assert(Sender <> nil);
  if Sender <> nil then
    if FUpdatingAppearance = 0 then
...
procedure TAppearanceListViewHorz.DoResetEditModeAnimation;
begin
  inherited;
  RefreshAppearances([TListItemPurpose.None]);
end;

procedure TAppearanceListViewHorz.BeginUpdate;
begin
...
procedure TAppearanceListViewHorz.EndUpdate;
begin
  inherited;
  if FUpdatingAppearance > 0 then
  begin
...
procedure TAppearanceListViewHorz.EditModeAppearances;
    ...
    try
      for Item in Filter.UnfilteredItems do
        if Item is TListViewItem and (Item.Purpose = TListItemPurpose.None) then
          PrepareEditModeObjects(TListViewItem(Item));
    finally
      EndUpdate;
    end;
  end;
...
procedure TAppearanceListViewHorz.PrepareEditModeObjects(const AItem: TListViewItem);
var
  LHandled: Boolean;
  LItemObjects: TItemAppearanceObjects;
begin
  ...
  try
    ...
    begin
      ...
      DoUpdatingItemView(AItem, LHandled);
      if not LHandled then
        LItemObjects.InitEditModeObjects(AItem, GetFinalItemSize);
      DoUpdateItemView(AItem);
    end;
  finally
    FResettingObjects := False;
...
function TAppearanceListViewHorz.GetItemHeight(const Index: Integer): Integer;
var
  Item: TListItem;
begin
  if (Index < 0) or (Index >= Adapter.Count) then
...
function TAppearanceListViewHorz.GetItemHeight: Integer;
begin
  Result := FItemAppearanceProperties.Height;
end;

function TAppearanceListViewHorz.GetItemEditHeight: Integer;
begin
  Result := FItemEditAppearanceProperties.Height;
end;

function TAppearanceListViewHorz.GetHeaderHeight: Integer;
begin
  Result := FHeaderAppearanceProperties.Height;
end;

function TAppearanceListViewHorz.GetFooterHeight: Integer;
begin
  Result := FFooterAppearanceProperties.Height;
end;

procedure TAppearanceListViewHorz.Resize;
begin
  inherited;
end;

procedure TAppearanceListViewHorz.DoButtonChange(const Sender: TObject; const AItem: TListItem;
const AObject: TListItemSimpleControl);
begin
  if Assigned(FOnButtonChange) then
    FOnButtonChange(Self, AItem, AObject);
end;

procedure TAppearanceListViewHorz.DoButtonClick(const Sender: TObject; const AItem: TListItem;
const AObject: TListItemSimpleControl);
begin
  if Assigned(FOnButtonClick) then
    FOnButtonClick(Self, AObject.Owner as TListItem, AObject);

  FClickEventControl := AObject;
  StartIncident(TDelayedIncident.ClickEvent);
end;

procedure TAppearanceListViewHorz.DoListItemClick(const AItem: TListItem);
begin
  inherited;
...
procedure TAppearanceListViewHorz.DoUpdateItemView(const AListItem: TListItem);
begin
  inherited;
...
procedure TAppearanceListViewHorz.DoUpdatingItemView(const AListItem: TListItem; var AHandled: Boolean);
begin
  inherited;
...
procedure TAppearanceListViewHorz.EditorBeforeItemAdded(Sender: IListViewEditor);
begin
  FItemSelectedBeforeEdit := Selected;
end;

procedure TAppearanceListViewHorz.EditorAfterItemAdded(Sender: IListViewEditor; const Item: TListItem);
begin
...
procedure TAppearanceListViewHorz.EditorBeforeItemDeleted(Sender: IListViewEditor; const Index: Integer);
begin
...
procedure TAppearanceListViewHorz.EditorAfterItemDeleted(Sender: IListViewEditor);
begin
  if (FItemSelectedBeforeEdit <> nil) and (ItemIndex <> FItemSelectedBeforeEdit.Index) then
    SetItemIndexInternal(FItemSelectedBeforeEdit.Index, True, True);
  FItemSelectedBeforeEdit := nil;
...
procedure TAppearanceListViewHorz.CheckableCheckChange(Sender: TObject);
begin
  DoListItemChange(Sender as TListItem);
end;
{$ENDREGION}
{$REGION 'TListView'}

procedure TListViewHorz.InitializeItemAppearance(
...
procedure TAdapterListViewHorz.SetAdapter(

  const Adapter: IListViewAdapter);
begin
  FAdapter := Adapter;
  if Adapter <> nil then
  begin
...
procedure TAdapterListViewHorz.ItemsChange(Sender: TObject);
begin
  DoItemsChange;
end;

procedure TAdapterListViewHorz.ItemsCouldHaveChanged(Sender: TObject);
begin
  DoItemsCouldHaveChanged;
end;

procedure TAdapterListViewHorz.ItemsMayChange(Sender: TObject);
begin
  DoItemsMayChange;
end;

procedure TAdapterListViewHorz.ItemsInvalidate(Sender: TObject);
begin
  DoItemsInvalidate;
end;

procedure TAdapterListViewHorz.ResetView(Sender: TObject);
begin
  DoResetView(Sender as TListItem);
end;

procedure TAdapterListViewHorz.ItemsResize(Sender: TObject);
begin
  DoItemsResize;
end;

procedure TAdapterListViewHorz.DoAdapterSet;
begin
end;

procedure TAdapterListViewHorz.DoItemsChange;
begin
end;

procedure TAdapterListViewHorz.DoItemsCouldHaveChanged;
begin
end;

procedure TAdapterListViewHorz.DoItemsMayChange;
begin
end;

procedure TAdapterListViewHorz.DoItemsInvalidate;
begin
end;

procedure TAdapterListViewHorz.DoResetView(const Sender: TListItem);
begin
end;

procedure TAdapterListViewHorz.DoItemsResize;
begin
  InvalidateHeights;
end;

procedure TAdapterListViewHorz.InvalidateHeights;
begin
...
procedure Register;
begin
  RegisterComponents('FMX-Ext', [TListViewHorz]);
end;

initialization

RegisterFmxClasses([TCustomListViewHorz, TListViewHorz]);
