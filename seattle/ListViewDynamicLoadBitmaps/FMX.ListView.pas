...
type
  ...
  strict private
    ...
    procedure SetSeparatorLeftOffset(const Value: Single);
    procedure SetSeparatorRightOffset(const Value: Single);
  protected
    procedure ApplyStyle; override;
  ...
  public
    ...
    property SeparatorLeftOffset: Single read FSeparatorLeftOffset
      write SetSeparatorLeftOffset; // ZuBy
    property SeparatorRightOffset: Single read FSeparatorRightOffset
      write SetSeparatorRightOffset; // ZuBy
...
function TAppearanceListView.getlastVisibleItemindex: Integer;
begin
  Result := TListViewBase(Self).Adapter.Count - 1;
end;

function TAppearanceListView.GetScrollWidth: Single;
begin
  Result := DefaultScrollBarWidth;
end;

function TAppearanceListView.getVisibleCount: Integer;
var
  I, c, n: Integer;
begin
  Result := 0;
  n := 0;
  c := TListViewBase(Self).ItemCount;
  if c > 1 then
  begin
    for I := getFirstVisibleItemIndex to c - 1 do
    begin
      n := n + TListViewBase(Self).GetItemHeight(I);
      Result := Result + 1;
      if n >= TListViewBase(Self).Height then
        Break;
    end;
  end;
end;
...
procedure TAppearanceListView.SetColorBackground(aColor: TAlphaColor);
begin
  FBackgroundStyleColor := aColor;
  Repaint;
end;

procedure TAppearanceListView.SetColorButtonText(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorDeleteText(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorHeader(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorItemFill(aColor: TAlphaColor);
begin
  FItemStyleFillColor := aColor;
  Repaint;
end;

procedure TAppearanceListView.SetColorItemFillAlt(aColor: TAlphaColor);
begin
  FItemStyleFillAltColor := aColor;
  Repaint;
end;

procedure TAppearanceListView.SetColorItemSelected(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorText(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorTextDetail(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorTextHeader(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorTextSelected(aColor: TAlphaColor);
begin
  FStyleResources.DefaultTextSelectedColor := aColor;
  Repaint;
end;

procedure TAppearanceListView.SetColorPullRefresh(aColor: TAlphaColor);
begin
...
procedure TAppearanceListView.SetColorStretchGlow(aColor: TAlphaColor);
begin
...
function TAppearanceListView.getFirstVisibleItemIndex: Integer;
var
  TopViewIndex: Integer;
begin
  TopViewIndex := Trunc(TListViewBase(Self).FScrollViewPos - TListViewBase(Self)
    .FSideSpace);
  Result := Min(Max(TListViewBase(Self).FindItemAbsoluteAt(TopViewIndex), 0),
