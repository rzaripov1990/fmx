...
begin
  Result := (AControl is TRectangle) and (AControl.Parent is TCommonCustomForm) and (TRectangle(AControl).Align = TAlignLayout.Contents);
end;
...
begin
  Result := (AControl is TLayout) and (AControl.Parent is TCommonCustomForm) and (StartsStr(sVKBHelperLayout, AControl.Name));
end;
...
begin
  if Assigned(FLastControl) then
    if (AComponent = FLastControl) and (Operation = TOperation.opRemove) or ((AComponent <> FLastControl) and (Operation = TOperation.opInsert)) then
      begin
    ...
    begin
      if ControlIsCommonLayout(TControl(AComponent)) then
        begin
          ...
          TControl(AComponent).SetBounds(0, 0, TCommonCustomForm(TControl(AComponent).Parent).ClientWidth, TCommonCustomForm(TControl(AComponent).Parent).ClientHeight);
        end;
...
begin
  ...
  if FocusedRect.IntersectsWith(FVKBounds) then
    AMovedLayout.SetBounds(AMovedLayout.Position.X, FVKBounds.Top - FocusedRect.Bottom, AMovedLayout.Width, AMovedLayout.Height);
end;
